# compile-executor-adapter.ps1 [-ExeName <name>] - csc merge pure-logic core + ExecutorAdapter to exe
# (NX File>Execute). 默认输出 ExecutorAdapter.exe；被 NX 会话占用时换名输出。
# 参数示例: powershell -NoProfile -File scripts\compile-executor-adapter.ps1 -ExeName ExecutorAdapter-v2.exe
param([string]$ExeName = 'ExecutorAdapter.exe')
