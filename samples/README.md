...
| `camprobe-chamferwrite-20260905-203052.txt` | **CHAMFER_MILL 直径写缺陷判定 + 修复（202730 复跑暴露）**：模板默认 ChamferLength=4（D16 型）→ D6 写直径被 NX 校验拒（s1-s3 全复现"交叉中心线"）；**预置 ChamferLength=D/2（90° 尖角）→ 写 D6 持久 ✓ 类型保持 MillChamfer**（s4/s5；ChamferLengthBuilder 为运行时反射面）→ ExecutorAdapter 按 subtype==CHAMFER_MILL 反射预写修复，复跑待 GUI Execute |
