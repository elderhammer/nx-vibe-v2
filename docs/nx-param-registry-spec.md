...
> 下游 v1.5-③ 参数面扩展按本表为红线依据——**已实现收官**（2026-09-04，见 nx-params-v15-spec.md
> 头部与 §2 #15 补行：v1.5-⑤ feed_cut 2026-09-05 三跑持久入白名单；#17 reference_tool / #18
> transfer_within_levels 2026-09-06 批次已实施，见 comparer spec §3 校准记录）。
...
| 9 | strategy.stepover.mode/value | `b.CutParameters.Stepover.StepoverType` / `.PercentToolFlatBuilder` | 腔 | 枚举 / Inheritable 叶子 | ✓ PercentToolFlat / 65、60 | **还原 ✗**（U-6 先行负结案，camprobe-stepover 三跑；主链 + StepoverLimit 均不可持久。2026-09-06 静态复核加固：2406 六新 option-menu 形态类零宿主零工厂零样例不可达，见索引 §2.5） |
...
| 12 | strategy.depth_per_cut | `b.CutLevel.GlobalDepthPerCut.DistanceBuilder`（**2026-09-05 v2.5 修正**：op 级 `b.DepthPerCut` 惰性——引擎不消费、gt 恒 0，见 camprobe-v2depth） | 腔 | Inheritable 叶子（DistanceBuilder） | ✓ 0.3/0.2/20/20（gt 实录 camprobe-v2surf/flip，v2depth 读回核对） | **持久 ✓ + 引擎消费 ✓**（camprobe-v2depth-211011 rebuilt 侧三 op：写 gt 值 commit → 新 builder 读回全持久，刀路长度按深度反比移动 0.3→×3.32 / 0.2→×4.85 / 20→×0.063；op 级写对照零变化 = 惰性坐实） |
| 13 | strategy.hole_depth（导出现状事实键，schema 未收——executor spec §1 偏差记录同源） | `b.HoleDepth.Value`（OperationBuilder 基类，NX5 头文件实证——PTP 与 DRILLING 重建均可达） | PTP + DRILLING | Inheritable 叶子 | ✓ 0（True=继承）/ 重建 DRILLING 0（False=executor 写 0 显式） | **重建落点（DRILLING）写面持久 ✓**（2026-09-06 camprobe-ptpkeys P1A 双会话：写 8.5 → commit → 新 builder 读回 8.5）；PTP 源 op 细分写面不做（approximation，见 executor spec）；孔族采集补读 hole_depth 键（PTP→DRILLING 近似键面对称，2026-09-06） |
...
| 18 | strategy.transfer_within_levels | `b.NonCuttingBuilder.TransferWithinLevelsType`（CavityMillingBuilder → NcmPlanarBuilder；C++ SetTransferWithinLevelsType Created NX5.0.0/License cam_base，CAM_NcmPlanarBuilder.hxx；.NET 反射 CanWrite=True 10 值；官方样例 PlanarOpsSetNCMCycleAll.vb 命中 = 经 op builder Commit 写范） | 腔 | 类嵌套枚举（NcmPlanarBuilder.TransferWithinLevelsTypes） | ✓ gt 四腔 op 全 Direct（v2surf-gt 195205 矩阵 + 探针复核），模板默认 Clearance | **持久 ✓ + 引擎消费 ✓**（2026-09-06 camprobe-v2ncm C1：写 Direct → 读回断言 OK + 36 区同构长度 4718→1380 收敛 71%；[I] 实录 143712：重建件 OP-002 3257→1299、OP-001 114682→111799） |
| 19 | strategy.transfer_within_levels_height（**负结案撤采**，schema 词典保留注记） | `b.NonCuttingBuilder.TransferWithinLevelsHeightBuilder`（.Value + .Intent，InheritableToolDepBuilder : InheritableDoubleBuilder；Intent NX5.0.0/cam_base） | 腔 | InheritableToolDep 叶子 + Intent | ✓ gt 0.5/PartUnits（四 op 同，v2surf-gt 195205） | **还原 ✗**（2026-09-06 camprobe-v2ncmh 四写序变体 H1-H4：Value 先/Intent 先/加 ValueIntent=PartUnits/加 ExpressionString="" 全组合 commit → 重开 Intent/ValueIntent 持久 ✓ 但 **Value 恒回模板 3**——UI 可设 API 不可写，stepover #9 同族新实例；处置 = 撤采（防永久假差，stepover 先例），[I] 143758 param=55/55 无此键差） |
...
| R-5 | 下游红线：重建白名单只含持久键（#1-4 增量 + 既有 #10-12 + **#15 feed_cut（v1.5-⑤，2026-09-05 三跑持久）** + **#17 reference_tool（2026-09-06 三重实证）** + **#18 transfer_within_levels（2026-09-06 持久+引擎消费）**）；#5-9/#19 拒收 + diag 不静默 | 实证口级纪律（executor spec PRE-4 维持） | v2.5 参考刀具批/转移族批实现 diff 审阅 | [doc] |
